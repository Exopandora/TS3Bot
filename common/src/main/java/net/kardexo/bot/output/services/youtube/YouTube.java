package net.kardexo.bot.output.services.youtube;
...
public class YouTube {
	...
	private static final SimpleCommandExceptionType ERROR_LOADING_VIDEO =
		new SimpleCommandExceptionType(new LiteralMessage("Error loading video"));
	private static final SimpleCommandExceptionType ERROR_LOADING_PLAYLIST =
		new SimpleCommandExceptionType(new LiteralMessage("Error loading playlist"));
	private static final SimpleCommandExceptionType ERROR_LOADING_PLAYLIST_ITEMS =
		new SimpleCommandExceptionType(new LiteralMessage("Error loading playlist items"));
	private static final SimpleCommandExceptionType ERROR_LOADING_PLAYLIST_LENGTH =
		new SimpleCommandExceptionType(new LiteralMessage("Error loading playlist length"));
	...
	public static JsonNode watch(IAPIKeyService apiKeyService, String id, String part) throws CommandSyntaxException {
		try {
			URI uri = new URIBuilder(API_URL.resolve("videos"))
				...
				.build();
			JsonNode node = OBJECT_MAPPER.readTree(uri.toURL().openStream());
	...
	public static JsonNode latestVideo(
		IAPIKeyService apiKeyService,
		String userId,
		Predicate<JsonNode> predicate,
		int skip
	) throws CommandSyntaxException {
		JsonNode videos = playlistItems(
			apiKeyService,
			uploadsPlaylistId(apiKeyService, userId),
			null,
			"snippet",
			MAX_RESULTS
		).path("items");
	...
	public static JsonNode playlistItems(
		IAPIKeyService apiKeyService,
		String playlist,
		String pageToken,
		String part,
		...
		try {
			...
			return OBJECT_MAPPER.readTree(builder.build().toURL().openStream());
		} catch (Exception e) {
