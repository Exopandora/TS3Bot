package net.kardexo.bot.output.services.twitch;
...
public class Twitch {
	...
	private static final SimpleCommandExceptionType TWITCH_SERVICE_UNAVAILABLE =
		new SimpleCommandExceptionType(new LiteralMessage("Twitch service is currently unavailable"));
		...
		try (CloseableHttpClient client = Util.httpClient()) {
			...
			try (CloseableHttpResponse response = client.execute(httpGet)) {
				...
				if (data != null && !data.isEmpty()) {
					...
					String result = content.path("user_name").asText()
						+ " is live for "
						+ content.path("viewer_count").asInt()
						+ " viewers "
						+ content.path("title");
					if (appendLink) {
