...
public class IntRangeArgumentType implements ArgumentType<IntRangeArgumentType.IntRange> {
	private static final SimpleCommandExceptionType INVALID_RANGE =
		new SimpleCommandExceptionType(new LiteralMessage("Upper bound must be higher than lower bound"));
	private static final Dynamic2CommandExceptionType INTERVAL_TOO_LOW =
		new Dynamic2CommandExceptionType(
			(found, min) -> new LiteralMessage("Lower bound must not be less than " + min + ", found " + found)
		);
	private static final Dynamic2CommandExceptionType INTERVAL_TOO_HIGH =
		new Dynamic2CommandExceptionType(
			(found, max) -> new LiteralMessage("Upper bound must be more than " + max + ", found " + found)
		);
	...
	public record IntRange(int lowerBound, int upperBound) {
	}
}
