...
public class CommandMessageProcessor implements IMessageProcessor {
	...
	public CommandMessageProcessor(
		...
		this.random = random;
		for (ICommandRegistrar registrar : ICommandRegistrar.INSTANCE) {
			registrar.register(
				this.dispatcher,
				bot,
				configService,
	...
	public boolean isApplicable(IBotClient bot, IChannel channel, IClient client, String message, ChatHistory chatHistory) {
		if (!message.startsWith(this.config.getCommandPrefix())) {
			return false;
		}
		if (this.config.getCommandPrefix().length() > 1) {
			return true;
		}
		return !message.matches("^(?:" + Pattern.quote(this.config.getCommandPrefix()) + ")+$");
	}
}
