...
import net.kardexo.bot.output.services.w2g.Watch2Gether;
import net.kardexo.bot.output.services.youtube.YouTube;
...
public class Watch2GetherCommand {
		...
		LiteralArgumentBuilder<CommandSource> command = Commands.literal("watch2gether")
			.executes(context ->
				watch2gether(
					...
					apiKeyService,
					urlMessageProcessor,
					config.getDefaultW2GShare(),
					config.getDefaultW2GBGColor(),
					config.getDefaultW2GBGOpacity()
				)
			)
			.then(Commands.argument("share", StringArgumentType.greedyString())
				.executes(context ->
					watch2gether(
						...
						apiKeyService,
						urlMessageProcessor,
						Util.extractURL(StringArgumentType.getString(context, "share")),
						config.getDefaultW2GBGColor(),
						config.getDefaultW2GBGOpacity()
					)
				));
			...
			command = command.then(Commands.literal(entry.getKey())
				.executes(context ->
					watch2gether(
						...
						apiKeyService,
						urlMessageProcessor,
						YOUTUBE_URL + YouTube.videoId(YouTube.latestVideo(apiKeyService, entry.getValue(), YouTube.createVideoDurationPredicate(config), 0)),
						config.getDefaultW2GBGColor(),
						config.getDefaultW2GBGOpacity()
					)
				)
				.then(Commands.literal("random")
					.executes(context ->
						watch2gether(
							...
							apiKeyService,
							urlMessageProcessor,
							YOUTUBE_URL + YouTube.videoId(YouTube.randomVideo(apiKeyService, entry.getValue())),
							config.getDefaultW2GBGColor(),
							config.getDefaultW2GBGOpacity()
						)
					))
						...
						watch2gether(
							...
							YOUTUBE_URL + YouTube.videoId(
								YouTube.latestVideo(
									apiKeyService,
									entry.getValue(),
									YouTube.createVideoDurationPredicate(config),
	...
	public static int watch2gether(
