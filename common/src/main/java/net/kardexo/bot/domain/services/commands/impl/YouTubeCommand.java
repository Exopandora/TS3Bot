...
public class YouTubeCommand {
		...
		for (Entry<String, String> entry : config.getShortcuts().getYoutube().entrySet()) {
			dispatcher.register(Commands.literal(entry.getKey())
				.executes(context ->
					youtube(
						context,
						YouTube.latestVideo(apiKeyService, entry.getValue(), YouTube.createVideoDurationPredicate(config), 0)
					)
				)
				.then(Commands.literal("random")
					.executes(context ->
						youtube(
							context,
							YouTube.randomVideo(apiKeyService, entry.getValue())
						)
					))
	...
	private static int youtube(CommandContext<CommandSource> context, JsonNode video) {
		...
		if (
			channelTitle != null && !channelTitle.isEmpty()
