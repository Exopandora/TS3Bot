...
import net.kardexo.bot.domain.services.commands.arguments.WordListArgumentType;
...
public class TeamsCommand {
	private static final Dynamic2CommandExceptionType CANNOT_SPLIT =
		new Dynamic2CommandExceptionType(
			(players, teamCount) -> new LiteralMessage("Cannot split " + players + " players into " + teamCount + " teams")
		);
		...
		dispatcher.register(Commands.literal("teams")
			.then(Commands.argument("team_count", IntegerArgumentType.integer())
				.executes(context ->
					teams(
						context,
						IntegerArgumentType.getInteger(context, "team_count")
					)
				)
				.then(Commands.argument("players", WordListArgumentType.list())
					.executes(context ->
						teams(
							context,
							IntegerArgumentType.getInteger(context, "team_count"),
							WordListArgumentType.getList(context, "players")
						)
					))));
	}
