...
public class KickCommand {
	private static final DynamicCommandExceptionType INVALID_USER =
		new DynamicCommandExceptionType(user -> new LiteralMessage("User " + user + " cannot be kicked"));
		...
		dispatcher.register(Commands.literal("kick")
			...
			.then(Commands.argument("username", ClientArgumentType.client(bot))
				...
				.then(Commands.argument("reason", StringArgumentType.greedyString())
					.executes(context ->
						kick(context, ClientArgumentType.getClient(context, "username"), StringArgumentType.getString(context, "reason"))
					))));
	}
